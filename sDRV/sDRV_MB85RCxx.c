...
static inline void write_byte(uint16_t addr,uint8_t byte){
    sBSP_I2C2M_MemSendByte(MB85RCxx_ADDR,addr,I2C_MEMADD_SIZE_16BIT,byte);
}
static inline uint8_t read_byte(uint16_t addr){
    return sBSP_I2C2M_MemReadByte(MB85RCxx_ADDR,addr,I2C_MEMADD_SIZE_16BIT);
}
static inline void write_bytes(uint16_t addr,uint8_t* pData,uint16_t length){
    sBSP_I2C2M_MemSendBytes(MB85RCxx_ADDR,addr,I2C_MEMADD_SIZE_16BIT,pData,length);
}
static inline void read_bytes(uint16_t addr,uint8_t* pData,uint16_t length){
    sBSP_I2C2M_MemReadBytes(MB85RCxx_ADDR,addr,I2C_MEMADD_SIZE_16BIT,pData,length);
}
