...
static inline void write_reg(uint8_t reg_addr, uint8_t data) {
    sBSP_I2C2M_MemSendByte(INA219_I2C_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, data);
}

// 读取寄存器
static inline uint8_t read_reg(uint8_t reg_addr) {
    return sBSP_I2C2M_MemReadByte(INA219_I2C_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT);
}

// 连续读取寄存器
static void read_regs(uint8_t reg_addr, uint8_t* pData, uint8_t len) {
    sBSP_I2C2M_MemReadBytes(INA219_I2C_ADDR, reg_addr, I2C_MEMADD_SIZE_8BIT, pData, len);
}

// 初始化INA219
void sDRV_INA219_Init() {
    sBSP_I2C2_Init(400000);

    uint16_t config = 0x399F; // 配置值需要根据具体应用调整
    uint16_t calibration = 819; // 计算得到的校准值
